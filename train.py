...
from loss import * 
from dataset import SegDataset as Dataset
from tools import *
    ...
    for epoch in range(args.epochs):
        ...
        for phase in ['train', 'valid']:
            ...
            if phase == 'train':
                ...
                model.train()
                train_loss = 0.0
                semantic_consistency_all = 0.0
                anatomical_consistency_all = 0.0
                smooth_loss_all = 0.0
                ...
                for i_batch, sample_batched in tqdm(train_t):
                    ...
                    if modality == 'CT':
                        rec_sim, flow_sim, weight_sim, _, flow_dif, flow, _, _, _, _, anatomical_consistency = model(atlas_ct.cuda(), atlas_mr.cuda(), target, modality,dice=True)
                        # save_nii(atlas_ct_label, None, None, "./wula.nii")
                        seg_sim = warp_train(atlas_ct_label.cuda(), flow_sim)
                        seg_dif = warp_train(atlas_mr_label.cuda(), flow_dif)
                    
                    if modality == 'MR':
                        rec_sim, flow_sim, weight_sim, _, flow_dif, flow, _, _, _, _, anatomical_consistency = model(atlas_mr.cuda(), atlas_ct.cuda(), target, modality,dice=True)
                    ...
                    loss_G.backward()
                    optimizer.step()
                    semantic_consistency_all += semantic_consistency * target.size(0)
                    anatomical_consistency_all +=  anatomical_consistency * target.size(0)
                    smooth_loss_all += smooth_loss * target.size(0)
...
if __name__ == '__main__':
    ...
    # Select atlas number
    atlas_filename = ['MR_19','MR_33','MR_37', 'CT_22','CT_25','CT_26']
