...
os.environ["CUDA_VISIBLE_DEVICES"] = '2' # Set your GPU
path ='../Dataset/CT_MR/' # Dataset path

weight_path = "../Weight/epoch_3626_dice_0.1382_.pth" # Your weight path
...
atlas_filename = ['MR_15','MR_19','MR_33','MR_37','MR_39','CT_6','CT_24','CT_25','CT_26','CT_29']
...
with torch.no_grad():
    for i_batch, sample_batched in enumerate(dataloader_valid):
        ...
        if modality == 'MR':
            target_segs = os.listdir('../Dataset/CHAOS/data_process/' + ID +'/T2SPIR/Ground')
            target_segs.sort()
            for i in range(len(target_segs)):
                if i == 0:
                    target_seg = mpimg.imread('../Dataset/CHAOS/data_process/' + ID +'/T2SPIR/Ground/'+target_segs[i])[...,np.newaxis]
                else:
                    mask = mpimg.imread('../Dataset/CHAOS/data_process/' + ID +'/T2SPIR/Ground/'+target_segs[i])[...,np.newaxis]
        ...
        if modality == 'CT':
            target_seg, header, affine, spacing = read_nii('../Dataset/BTCV/data/label_'+str(int(ID))+'.nii.gz')
        ...
        if modality == 'CT':
            save_nii(registered_seg, header, affine, f"../Result/regi{ID}.nii")
            save_nii(target_seg, header, affine, f"../Result/targ{ID}.nii")
